...
export function Button({ small = false, gray = false, className = "", ...props }: ButtonProps) {
    ...
    return <button className={`text-white rounded-full transition-colors duration-200 disabled:cursor-not-allowed disabled:opacity-50 ${sizeClasses} ${colorClasses} ${className}`} {...props} />;
